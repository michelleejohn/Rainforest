...
bubble_font = pygame.font.SysFont("segoescript,brushscriptmt,lucidahandwriting,arial", 20)
button_font = pygame.font.SysFont("segoescript,brushscriptmt,lucidahandwriting,arial", 12)
ending_font = pygame.font.SysFont("segoescript,brushscriptmt,lucidahandwriting,arial", 34)
small_font = pygame.font.SysFont("segoescript,brushscriptmt,lucidahandwriting,arial", 20)

# rectangles
title_rect = pygame.Rect(20, 20, 180, 55)

bubble_rect = pygame.Rect(0, 90, 430, 150)
bubble_rect.centerx = WIDTH // 2

choice_rect = pygame.Rect(0, 0, 180, 150)
...
def draw_scene(time_ms):
    ...
    temp = pygame.Surface((bubble_rect.width, bubble_rect.height), pygame.SRCALPHA)
    for i in range(bubble_rect.height):
        t = i / max(bubble_rect.height - 1, 1)
        r = int(79 + (166 - 79) * t)
        g = int(138 + (208 - 138) * t)
        b = int(79 + (141 - 79) * t)
        pygame.draw.line(temp, (r, g, b), (0, i), (bubble_rect.width, i))

    rounded_mask = pygame.Surface((bubble_rect.width, bubble_rect.height), pygame.SRCALPHA)
    ...
    screen.blit(temp, bubble_rect.topleft)

    arrow_points = [
        (bubble_rect.centerx - 18, bubble_rect.bottom),
        (bubble_rect.centerx + 18, bubble_rect.bottom),
        (bubble_rect.centerx, bubble_rect.bottom + 18)
    ]
    ...
    for fairy in fairies:
        if fairy["image"]:
            ...
            img_rect = fairy["image"].get_rect(topleft=(fairy["x"], fairy["base_y"] + float_offset))
...
def draw_keep_screen():
    screen.fill((224, 255, 224))
    draw_text(screen, "You kept your wings!", ending_font, TEXT_DARK, WIDTH // 2, 220, center=True)
    draw_text(screen, "Mariposa welcomes you as a fairy.", small_font, TEXT_DARK, WIDTH // 2, 290, center=True)
...
def draw_return_screen():
    ...
    mouse_pos = pygame.mouse.get_pos()
    draw_button(screen, again_button, "Play Again", mouse_pos)
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            elif current_screen in ("keep", "return"):
